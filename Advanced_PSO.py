...
#pos[2, :] = np.array([5, 2, 5, 1, 1.0])    #CL, I, W1, W3, W5
...
for i in range(n):
  for j in range (m):
    pos[i,j]=LB[j]+np.random.rand(1,1)[0]*(UB[j]-LB[j]);
...
for chay in range(lap):
  ...
  fitness, ketqua = fitness_run.calFitness(pos)
...
df.to_csv("ketquaPSOv2_4.csv")
df.to_excel("ketquaPSOv2_4.xlsx", sheet_name="PSO")
#ve dang song		
pos[0, :] = np.transpose(gbest_pos)   #CL, I, W1, W3, W5
#os.system("ocean -restore final_PSO2.ocn")
